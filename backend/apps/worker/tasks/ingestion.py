...
async def _reassess_scheduled_cases_async(
    session_factory: Optional[async_sessionmaker] = None,
) -> Dict[str, Any]:
    """Async execution body for scheduled cases reassessment."""
    if session_factory is None:
        settings = get_settings()
        session_factory = get_sessionmaker(settings.DATABASE_URL)
    ...
    async with session_factory() as session:
        async with session.begin():
            stmt = (
                ...
                .where(
                    RecoveryCaseModel.status == RecoveryCaseStatus.WAITING_EXTERNAL,
                    RecoveryCaseModel.next_action_scheduled_at.is_not(None),
                )
                .with_for_update()
            )
            cases = (await session.execute(stmt)).scalars().all()
            for case in cases:
                if case.next_action_scheduled_at is not None:
                    sched_at = case.next_action_scheduled_at
                    if sched_at.tzinfo is None:
                        sched_at = sched_at.replace(tzinfo=timezone.utc)
                    if sched_at <= now:
                        case.status = RecoveryCaseStatus.DETECTED
                        case.next_action_scheduled_at = None
                        reassessed_count += 1
                        logger.info(
                            f"Case '{case.id}': Scheduled wait delay expired. Transitioned to DETECTED."
                        )

    return {"status": "success", "reassessed_cases_count": reassessed_count}


@celery_app.task(name="tasks.ingestion.reassess_scheduled_cases")
def task_reassess_scheduled_cases() -> Dict[str, Any]:
    """
    Periodic or triggered worker task that identifies scheduled cases whose delay has expired.
    """
    logger.info("Executing task_reassess_scheduled_cases")
    return asyncio.run(_reassess_scheduled_cases_async())
